...
int q1(char data[]) {
  ...
  // Vaidação de mês com 30 ou 31 dias
  if (dq.iMes < 8) { // Antes de agosto, todos os meses com 31 dias são ÍMPARES
    if (dq.iMes % 2 == 0) {
      if (dq.iDia > 30) {
        dq.valido = 0;
        return dq.valido;
  ...
  } else { // A partir de agosto, todos os meses com 31 dias são PARES
    if ((dq.iMes % 3) == 0) {
      if (dq.iDia < 30) {
        dq.valido = 0;
        return dq.valido;
...
/*
 ...
 valores abaixo 1 -> cálculo de diferença realizado com sucesso 2 ->
 datainicial inválida 3 -> datafinal inválida 4 -> datainicial > datafinal
    Caso o cálculo esteja correto, os atributos qtdDias, qtdMeses e qtdAnos
 devem ser preenchidos com os valores correspondentes.
 */
...
/*
 ...
 armazenar no vetor, da seguinte forma: posicoes[0] = 13; posicoes[1] = 16;
        Observe que o índice da posição no texto deve começar ser contado a
